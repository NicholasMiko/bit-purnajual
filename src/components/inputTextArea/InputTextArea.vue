  ...
  <div class="grid w-full grid-rows-1 gap-2">
    <div>
      <textarea
        ...
        class="h-24 w-full resize-none rounded-md border p-2 text-sm shadow-xs focus:outline-none focus:ring-1"
        :class="[
          disabled
      ...
      />
      <div v-if="maxLength > 0" class="mt-1 text-xs text-ink-500">
...
function refreshDisabledRule() {
  const defaultRule = withDefaultGeneralInputTextRule(true, props.required, props.allowSlash, props.label || props.name)
